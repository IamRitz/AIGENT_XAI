...
def segmentation_fn(image):
    ...
    return slic(image, n_segments=40, compactness=10)
...
def loadImage(img_path, processed=True):
    ...
    if not processed:
        ...
        print("here2")
        img_array = np.reshape(img_path, (28, 28, 1))
...
def limeExplanation(model, img_path, MODEL_PATH=False):
    ...
    img_array_rgb, img_array_reshaped = loadImage(img_path, True)
    ...
    # Generate an explanation for the instance
        explanation = explainer.explain_instance(img_array_rgb[0].astype('double'), predict_fn, top_labels=5, hide_color=0, num_samples=1000, segmentation_fn=segmentation_fn)
    ...
    print(list(important_segments.values()))

    return list(important_segments.values())

if __name__ == '__main__':
    importance_bundle = limeExplanation(model=None, img_path='../Images/OriginalImages_max/Image_0.jpg', MODEL_PATH='../Models/mnist.h5')
