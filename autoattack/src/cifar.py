...
def cifar_model():
    # declare variables
    model_layers = [ tf.keras.layers.Input(shape=(32,32,3), name="model/input"),
...
def getAdversarial(threshold):
    ...
    # tf_model.save('models/cifar_autoattack.h5')

    # If you already have a model, use the line below
    tf_model = tf.keras.models.load_model('models/cifar_autoattack.h5')
...
def calculateNorms(orig_o, adv_o, orig, adv):
    model = tf.keras.models.load_model('models/cifar_autoattack.h5')
