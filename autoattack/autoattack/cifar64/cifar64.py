...
train_img_path = "./cifar10-64/train"
test_img_path = "./cifar10-64/test"
...
def cifar_model():
    # declare variables
    model_layers = [ tf.keras.layers.Input(shape=(64,64,3), name="model/input"),
...
def getAdversarial(threshold):
    ...
    #tf_model.save('models/cifar64_autoattack.h5')

    # If you already have a model, use the line below
    tf_model = tf.keras.models.load_model('models/cifar64_autoattack.h5')
...
def calculateNorms(orig_o, adv_o, orig, adv):
    model = tf.keras.models.load_model('models/cifar64_autoattack.h5')
...
def save_images(images, folder):
    ...
    for i, img_array in enumerate(images):
        #img = Image.fromarray(np.transpose((img_array.squeeze() * 255).astype(np.uint8), (1, 2, 0)))
...
if __name__ == '__main__':
    ...
    save_images(adv, "../Images/Adversarial2_cifar64")
    save_images(orig, "../Images/Original2_cifar64")
    t2 = time()
    print("\n\n######################################################################")
