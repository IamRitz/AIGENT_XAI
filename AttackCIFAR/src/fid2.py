...
def loadModel():
    model = tf.keras.models.load_model(os.path.abspath(os.path.join(os.getcwd(), os.pardir)) +'/Models/CIFAR5/cifar_64.h5')
    return model
...
def getImages():
    ...
    im1 = im1.reshape((im1.shape[0], 64*64*3)).astype('float32')
    im2 = im2.reshape((im2.shape[0], 64*64*3)).astype('float32')
