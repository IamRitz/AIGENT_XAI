...
class findCorrection:
    ...
    def run(self, model_name, num):
        ...
        second_lastlayer_inputs = np.load('./data/{}.lastlayer.input.npy'.format(orig_model_name))
        if num >= 0:
            second_lastlayer_inputs = second_lastlayer_inputs[1]
        print("Last layer: ", second_lastlayer_inputs)
        print("The original model picked was: ",filename)
        network = read_tf_weights_as_var(filename=filename, inputVals=second_lastlayer_inputs)
        # print(network.epsilons)
        # print(type(network))
        # print("Output Vars:",network.outputVars)
        unsat_epsilon, sat_epsilon, sat_vals = self.findEpsilon(network) if self.lp else self.findEpsilonInterval(network, second_lastlayer_inputs)
